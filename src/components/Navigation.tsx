  ...
  return (
    <div className="fixed z-50 bottom-8 left-0 right-0 flex justify-center items-end pointer-events-none px-4">
      <motion.nav 
        ...
        className="pointer-events-auto bg-white/10 dark:bg-black/20 backdrop-blur-3xl rounded-full px-6 py-3 flex gap-4 items-center shadow-2xl shadow-purple-900/20 dark:shadow-black/50 ring-1 ring-white/20 dark:ring-white/10"
      >
        {menuItems.map((item) => {
            ...
            >
              <div
                className={cn(
                  ...
                  isActive 
                    ? "text-purple-600 dark:text-purple-300 scale-110 -translate-y-1" 
                    : "text-slate-500 dark:text-slate-400 hover:text-purple-500 dark:hover:text-purple-300 hover:scale-110"
                )}
              >
              ...
              <div className="absolute bottom-full mb-4 hidden md:block opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none">
                <div className="bg-slate-900/90 dark:bg-purple-100/90 backdrop-blur-md text-white dark:text-purple-900 text-[10px] font-bold px-3 py-1.5 rounded-lg shadow-xl whitespace-nowrap transform translate-y-2 group-hover:translate-y-0 transition-transform">
                  {item.label}
                  <div className="absolute top-full left-1/2 -ml-1 border-4 border-transparent border-t-slate-900/90 dark:border-t-purple-100/90" />
                </div>
              </div>
            </button>
          );
